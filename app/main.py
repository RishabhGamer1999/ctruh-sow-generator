"""
CTRUH SOW Generator — Main Application
"""
import os
import time
import tempfile
...
from document.parser import parse_requirement_doc, extract_raw_text
...
with st.sidebar:
    ...
    # ── Upload Requirement / Account Proposal Doc ─────────────────────────────
        ...
        try:
            ...
            if extracted:
                ...
                if missing:
                    ...
                    lines.append(f"\n👉 **Next step:** {_next_question(next_field)}")
                else:
...
# Download banner when document is ready
...
if user_input := st.chat_input("Type your message or project details here..."):
    ...
    # Response generation with live timer
    with st.chat_message("assistant", avatar="🤖"):
        status_box = st.empty()
        t0 = time.time()
        
        status_box.info("🧠 Thinking & retrieving CTRUH context... (0.0s)")
        
        # Periodic UI update while generating
